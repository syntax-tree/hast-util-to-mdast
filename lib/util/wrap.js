 ...
 * content as-is.
 *
 * @template {BlockContent | PhrasingContent} NodeType
 *   Node type.
 * @param {Array<NodeType>} nodes
 *   Content.
 * @returns {Array<Exclude<BlockContent, PhrasingContent> | BlockContent>}
...
/**
 * @template {Delete | Link} NodeType
 * @param {NodeType} node
 * @returns {Array<BlockContent>}
 */
function split(node) {
  // @ts-expect-error: yeah, delete and link is weird.
   ...
   *
   * @template {BlockContent} ChildType
   * @param {ChildType} child
   * @returns {NodeType | ChildType}
   */
  function onnonphrasing(child) {
    if ('children' in child && 'children' in node) {
      const newParent = cloneWithoutChildren(node)
      /** @type {ChildType} */
      // @ts-expect-error: yes, that’s a child type.
      const newChild = cloneWithoutChildren(child)
      // @ts-expect-error: assume `child.children` can be inside `node`.
      newParent.children = child.children
      // @ts-expect-error: assume `node` can be inside `child`.
 ...
 * @template {BlockContent | PhrasingContent} NodeType
 *   Node type.
 * @template {BlockContent} MapType
 *   Node type.
 * @template {BlockContent} OtherMapType
 *   Node type.
 * @param {Array<NodeType>} nodes
 *   List of input nodes.
 * @param {(nodes: Array<PhrasingContent>) => Array<MapType>} onphrasing
 *   Turn phrasing content into block content.
 * @param {(node: BlockContent) => OtherMapType} onnonphrasing
 *   Map block content (defaults to keeping them as-is).
 * @returns {Array<BlockContent | MapType | OtherMapType>}
 */
function runs(nodes, onphrasing, onnonphrasing) {
  const flattened = flatten(nodes)
  /** @type {Array<BlockContent | MapType | OtherMapType>} */
  const result = []
...
function flatten(nodes) {
  ...
  while (++index < nodes.length) {
    ...
    if (
      ...
      // @ts-expect-error: correct, that’s unexpected.
